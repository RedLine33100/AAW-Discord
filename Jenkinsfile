      ...
      steps{
          ...
          dir("backend"){
            ...
            script {
              ...
              echo "B3"
              def now = new Date()
              sh 'docker run -d --name apibackend -p 8000:8000 . &> /logs/apibackend'+now.format("yyMMdd_HHmm", TimeZone.getTimeZone('UTC+1'))+'.log'
          ...
          dir("frontend"){
            ...
            script {
              ...
              echo "C3"
              def now = new Date()
              sh 'docker run -d --name aawfrontend -p 3777:3777 . &> /logs/aawfrontend'+now.format("yyMMdd_HHmm", TimeZone.getTimeZone('UTC+1'))+'.log'
